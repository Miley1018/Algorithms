 ...
 */
public class C24MergeDownUp2<Item extends Comparable<Item>> {
    private Item[] copy;
    public void sort(Item[] a){
        copy =(Item[]) new Comparable[a.length];
        for (int sz = 1; sz<a.length;sz = sz*2){
            for (int j = 0; j+sz<a.length;j+=sz*2){//important
                merge(a,j,j+sz-1,Math.min(j+sz+sz-1,a.length-1));
            }
        }
    }
    ...
    public static void main(String[] args){
        C24MergeDownUp2 sort = new C24MergeDownUp2();
